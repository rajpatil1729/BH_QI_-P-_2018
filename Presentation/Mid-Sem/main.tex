...
\date{$16^{th}$ October, 2018 \\ ~ \\ ~ \\\scriptsize Midsem Presentation\\ for \\ Semester Project PHY401 (Fall-2018) }
...
\begin{frame}{Penrose Diagram}{Motivation}
	 %To study furter properties of BH we need to know this concept
	
	\begin{mdframed}
		\centering 
		Given a manifold M and metric g, we want to draw a two-dimensional diagram capturing the causal relations between different points in spacetime. 
	\end{mdframed}
	~\\
	\pause
	\flushleft
	...
	~\\
	\scriptsize
	$ \bullet ~~~ $Coordinate transform (M,g) to bring it in finite co-ordinate distance \\~~~~~(metric may diverge at infinity)\\
	\vspace{0.1cm}
	$ \bullet ~~~ $Make a conformal transformation on the metric g to g' which makes\\~~~~~it regular on the edges.\\
	\vspace{0.1cm}
	$ \bullet ~~~ $Add points at infinity to M to make it M'\\
	\vspace{0.1cm}
	$ \bullet ~~~ $(M',g') is the conformal compactification(Penrose Diagram) of (M,g)
	
\end{frame}
%\begin{frame}{Penrose Diagram}{Minkowski Metric in 1+1 dim}
%	\begin{columns}
%		\begin{column}{0.5\textwidth}
%			\scriptsize
%			$ \bullet ~~~ $ $ds^2 = -dt^2+dx^2$\\
%			\vspace{0.1cm}
%			$ \bullet ~~~ $ $ u = t-x $ and $ v = t+x $\\
%			\vspace{0.1cm}
%			$ \bullet ~~~ $ $ \bar{u} = \tan^{-1}(u) $ and $ \bar{v} = \tan^{-1}(v) $\\
%			\vspace{0.1cm}
%			$ \bullet ~~~ $ $ ds^2 = \frac{-1}{\big( \cos\bar{u} \cos\bar{v} \big)^2} ~d\bar{u}~d\bar{v} $\\
%			\vspace{0.1cm}
%			$ \bullet ~~~ $ $ d\bar{s}^2 = \big( \cos\bar{u} \cos\bar{v} \big)^2 ds^2 $\\
%			\includegraphics[width=0.7\textwidth]{./Img/Min_11_PD1.png} \\
%			$ \bullet ~~~ $Now we add points $\bar{u},\bar{v} = \pm \frac{\pi}{2}$
%		\end{column}
%		\begin{column}{0.6\textwidth}  %%<--- here
%			\includegraphics[width=1\textwidth]{./Img/Min_11_PD2.png}	
%		\end{column}
%	\end{columns}
%\end{frame}

\begin{frame}{Penrose Diagram}{Minkowski Metric in 3+1 dim}
	\begin{columns}
		\begin{column}{0.6\textwidth}
			\scriptsize
			$ \bullet ~~~ $ $ds^2 = -dt^2+dr^2 + r^2 d\Omega^2$\\
			\vspace{0.1cm}
			$ \bullet ~~~ $ $ u = t-r $ and $ v = t+r $\\
			\vspace{0.1cm}
			$ \bullet ~~~ $ $ \bar{u} = \tan^{-1}(u) $ and $ \bar{v} = \tan^{-1}(v) $\\
			\vspace{0.1cm}
			$ \bullet ~~~ $ $ ds^2 = \frac{-1}{\big(2 \cos\bar{u} \cos\bar{v} \big)^2} \big[ -4~d\bar{u}~d\bar{v} + \sin^2(\bar{v}-\bar{u}) d\Omega^2 \big] $\\
			\vspace{0.1cm}
			$ \bullet ~~~ $ $ d\bar{s}^2 = \big( \cos\bar{u} \cos\bar{v} \big)^2 ds^2 $\\
			\vspace{0.1cm}
			$ \bullet ~~~ $ $ r \ge 0 ~~ \implies \bar{u}\le\bar{v} $\\
			\vspace{0.1cm}
			$ \bullet ~~~ $Now we add points $\bar{u},\bar{v} = \pm \frac{\pi}{2}$
		\end{column}
		\begin{column}{0.4\textwidth}  %%<--- here
			\includegraphics[width=1\textwidth]{./Img/Schawrzschild_PD.png}	
		\end{column}
	\end{columns}
\end{frame}


\begin{frame}{Penrose Diagram}{Kruskal Metric in 3+1 dim}
	\begin{columns}
		\begin{column}{0.3\textwidth}
			\includegraphics[width=1\textwidth]{./Img/Kruskal_M.JPG}
			\vspace{3cm}
...
\begin{frame}{Unruh Effect}{Constantly accelerated observer}
	Minkowski metric (2 dim)  : $ ds^2 = -dt^2 +dx^2 $\\
	\vspace{0.5cm}Claim : the trajectory of a constanly accelerated observer with acceleration 'a' is Hyperbolic in Minkowski co-ordinates. \\	
	\vspace{0.3cm}
	\pause
	Proof:\\
	\begin{columns}
	\begin{column}{0.5\textwidth}
		...
		~\\
		\centering
		$ t(\eta)=\frac{1}{a} \sinh(a\eta) $\\% in region 1
		\vspace{0.2cm}
		$ x(\eta)=\frac{1}{a} \cosh(a\eta) $\\% in region 1
		
	\end{column}
	\begin{column}{0.5\textwidth}  %%<--- here
		~\\
		\vspace{0.2cm}
		$~~~~ \bullet ~~ $Proper Acceleration :\\
		~\\
		\centering
		$a^{\mu} = \frac{d^2x^{\mu}}{d^2\eta} $\\%this actually should be proper accelaration or explain it properly
		\scriptsize
		~\\
		$a^t = a \sinh(a\eta) $ and $a^x = a \cosh(a\eta) $
	\end{column}
	\end{columns}
	\vspace{0.5cm}
	\centering
	$ |a| = \sqrt{a^\mu a_\mu} = \sqrt{-a^2\sinh^2(a\eta) + a^2 \cosh^2(a\eta)} = a  $\\
	\vspace{0.2cm}
	$\boxed{\text{Trajectory :}~~~~ x^2 - t^2 = \frac{1}{a^2} }$
\end{frame}
...
\begin{frame}{Hawking Radiation}{}
	\centering
	Unruh Effect $\implies$ Concept of Vaccum is Observer dependent
	\vspace{1cm}
	\pause
	\begin{mdframed}
	\begin{columns}
